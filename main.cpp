...
const double eps = 1e-4;
	...
	double det(Matrix a) {
		...
		k = 1; //(-1) в степени i
		n = a.m - 1;
		if (a.m < 1) cout << "Impossiиду to count determinant";
		if (a.m == 1) {
			d = a.vvi[0][0];
			return d;
		}
	...
	Vector solveCG(Matrix a, Vector b) {
		...
		if (isPosDetermined(a) == 1)
		{
			...
			cout<<"Not positively determined "<<endl;
	}
